...
$(document).ready(function () {
    ...
    $(".del").on("click", function () {
        ...
        layer.confirm('你确定要删除?', {icon: 3, title:'提示'}, function(index){
            var loading = layer.load(2, {shade: [0.5]}); //0代表加载的风格，支持0-2
            $.ajax({
                url: url,
                dataType: 'json',
                data: {'id': id},
    ...
    $(".page-search-btn").on("click", function () {
        var result = $(".page-search-form").serializeArray();
        var data = '';
        ...
        var loc = window.location.origin + window.location.pathname + "?" + data.substr(0, data.length - 1);
