...
module.exports = {
     ...
     */
    getPosition(sequence, relativeSequence){
        let result = sequence - relativeSequence;
        ...
        //sequence  方位永远是南
        let position = ["south", "east", "north", "west"];

        if (result < 0){
            return position[Math.abs(result)];
        }

        if (result > 0){
            return position[relativeSequence];
        }
    ...
    termDate(date , month, format){
        ...
        return moment(date).subtract(month, 'months').format(format);
    },
    offsetTime(date, time, type = 'minutes', format = "YYYY-MM-DD HH:mm:ss"){
